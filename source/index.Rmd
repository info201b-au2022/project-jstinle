...
According to our summary information, the age group with the highest unpaid loans are borrowers between `r summary_info$max_num_loans_age` with an in total of `r summary_info$max_num_loans` within our dataset populations. The age group `r summary_info$min_num_loans_age` has the fewest amount of loans with `r summary_info$min_num_loans` highlighting a correlation of higher debts in borrowers within the lower and higher education average age range.

In the age group `r summary_info$max_num_borrowers_age`, there are `r summary_info$max_num_borrowers`, the highest proportion of borrowers relative to any other age groups. This highlights a greater amount of borrowers with in this age group, indicating a correlation between higher frequency in loans compared to other age groups.

The highest amount of debt from borrowers `r summary_info$highest_bachelor_debt`is from the age group `r summary_info$highest_bachelor_debt_age`, showcasing that the population with the greatest amount of debt also correlates with the group with the greatest frequency of bachelor degrees. On the contrary the lowest outstanding debt `r summary_info$lowest_bachelor_debt` is from `r summary_info$lowest_bachelor_debt_age` age group indicating a there is less debt and borrowers relative to the `r summary_info$max_num_borrowers_age` age group.
